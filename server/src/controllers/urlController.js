...
const generateUrl = async function (req, res) {
  try {
    ...
    if (!isUrlValid(longUrl)) {
      return res.status(400).send({ status: false, message: 'longUrl is not valid. Please provide a valid URL' });
    }

    let cachedUrlData = await GET_ASYNC(`${longUrl}`);
    if (cachedUrlData) {
      const urlDetails = JSON.parse(cachedUrlData);
      return res.status(200).send({ status: true, data: urlDetails, msg: 'URL is coming from Cache' });
    }
    ...
    if (url) {
      ...
      return res.status(200).send({ status: true, data: url, msg: 'URL is coming from DB' });
    } else {
